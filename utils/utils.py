...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    return {'weights': model.to(device), 'processor': processor}
...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=128):
    # Number of samples per batch, --> 128 roughly takes 4 GB of GPU memory for florence v2 weights
    to_pil = ToPILImage()
    if starting_idx:
    ...
    model, processor = caption_model_processor['weights'], caption_model_processor['processor']
    if not prompt:
...
def predict(model, image, caption, box_threshold, text_threshold):
    """ Use huggingface weights to replace the original weights
    """
    model, processor = model['weights'], model['processor']
    device = model.device
...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=128):
    ...
    if use_local_semantics:
        caption_model = caption_model_processor['weights']

...
def run(theme_image_path: str, theme_json_path: str, default_image_path: str, default_json_path: str, issue_output_dir: str):
    """
    레이아웃 분석을 실행하는 함수
    
    Args:
        theme_image_path (str): 테마 이미지 경로
        theme_json_path (str): 테마 JSON 파일 경로
        default_image_path (str): 기본 이미지 경로
        default_json_path (str): 기본 JSON 파일 경로
    """
    try:
        # SingleLayout 분석
        ...
        issue_output_path = os.path.join(issue_output_dir, "issue")
        with open(issue_output_path, "w", encoding="utf-8") as f:
            json.dump(single_issues, f, ensure_ascii=False, indent=2)
        ...
        with open(issue_output_path, "a", encoding="utf-8") as f:
            json.dump(compare_issues, f, ensure_ascii=False, indent=2)

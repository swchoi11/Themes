...
class Visualizer():
    ...
    def visualize_skeleton_result(self, image_path: str, result_path: str, save_path: str):
        """스켈레톤 결과를 시각화합니다."""
        # 결과 로드
        ...
        self._visualize_all_elements(image, result, save_path)

        # # 2. 레이아웃 영역별 시각화
        # self._visualize_layout_regions(original_image.copy(), result, os.path.join(output_dir, '02.layout_regions.png'))
        # self._visualize_layout_regions(image, result, save_path)
...
def visualize_ui_skeleton_result(image_path: str, result_path: str, output_dir: str, cluster_output_name: str):
    """UI 스켈레톤 결과 시각화 편의 함수"""

    save_dir = os.path.join(output_dir, cluster_output_name)
    ...
    report_path = os.path.join(output_dir, f"{os.path.splitext(cluster_output_name)[0]}_report.txt")
    # visualizer.export_detailed_analysis(result, report_path)

    print(f"시각화 완료: 결과는 {save_dir}에서 확인하세요.")
    print("생성된 파일: 스켈레톤 구조")

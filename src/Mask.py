...
class Mask:
    ...
    def mask_image(self, img: Union[str, np.ndarray]) -> Tuple[np.ndarray, List[Dict[str, Any]]]:
        ...
        masked_image = np.full(img.shape, mask_color, dtype=np.uint8)
        all_rows = []
    ...
    def mask_directory_images(self, image_dir: str, is_gray: bool) -> None:
        ...
        if not image_list:
            print(f"[WARN] {image_dir} 디렉토리에서 이미지를 찾을 수 없습니다.")
            return
        ...
        for image_path in image_list:
            ...
            try:
                ...
                if not is_gray:
                    output_path = os.path.join(image_dir, f"masked_{image_name}")
                    cv2.imencode('.png', masked_img)[1].tofile(output_path)
                    print(f"마스킹된 이미지 저장됨: {output_path}")
                else:
                    gray_img = cv2.cvtColor(masked_img, cv2.COLOR_BGR2GRAY)
                    output_path = os.path.join(image_dir, f"masked_gray_{image_name}")
                    cv2.imencode('.png', gray_img)[1].tofile(output_path)
                    print(f"흑백 이미지 저장됨: {output_path}")
            except Exception as e:
                print(f"[ERROR] 이미지 처리 중 오류 발생 {image_path}: {str(e)}")

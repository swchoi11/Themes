...
class make_base_template():
    def __init__(self, min_box_size: int, iou_threshold: float):
        # 설정값 (10 0.5)
    ...
    def resize_image(self, image: np.ndarray) -> np.ndarray:
        ...
        if w <= 1800:
            target_size = (1080, 2376)
        else:
            target_size = (1856, 2176)
    ...
    def extract_boxes(self, image: np.ndarray) -> List[Tuple[int, int, int, int]]:
        ...
        for cnt in contours:
            ...
            if self.MIN_BOX_SIZE <= w and self.MIN_BOX_SIZE <= h:
    ...
    def compute_overlap_heatmap(self, all_boxes: List[List[Tuple[int, int, int]]],
                                 image_shape: Tuple[int, int], min_overlap_count: int=2,
    ...
    def run(self):
        for cluster_folder in sorted(os.listdir(self.cluster_dir)):
            ...
            for p in image_paths:
                img = make_base_template.safe_imread(p)
                if img is None:
            ...
            sample_img = make_base_template.safe_imread(image_paths[0])

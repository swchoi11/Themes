...
class Classification:
    ...
    @timefn
    def get_json(self, image_path: str, json_base_path: str):
        """
        이미지에 대응되는 json 파일 경로를 반환
        """
        filename = os.path.splitext(os.path.basename(image_path))[0]
        json_path = os.path.join(json_base_path, f'{filename}.json')
        if not os.path.exists(json_path):
            logger.warning(f"[WARNING] 대응되는 JSON 파일이 존재하지 않습니다: {json_path}")
            return ""
        return json_path
        
    
    def run(self, target_image_dir: str, default_image_dir: str, default_json_dir: str):
        # default_image_path: 복사할 원본 파일 경로 (파일 단위)
        # default_image_dir: 복사된 파일을 저장할 폴더 경로
        ...
        for target_image_path in image_list:
            ...
            output_path = os.path.join(default_image_dir, f"default{index}_{default_filename}")
            shutils.copy2(default_image_path, output_path)

            # json 복사
            default_json_path = self.get_json(default_image_path)
            if default_json_path:
                default_json_name = os.path.basename(default_json_path)
                output_json_path = os.path.join(default_json_dir, f"default{index}_{default_json_name}")
                shutils.copy2(default_json_path, output_json_path)                

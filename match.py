...
def split_by_ratio(image_dir):
    ...
    """
    image_list = glob.glob(f"{image_dir}/*.png", recursive=True)
    for image_path in image_list:
        image = cv2.imread(image_path)
        height, width = image.shape[:2]

        dest_dir = f"./output/xml-bbox/ratio_{width}_{height}"
        os.makedirs(dest_dir, exist_ok=True)
        image_name = os.path.basename(image_path)

    # return _layout_list("./output/xml-bbox")
...
def mask_images(image_dir, output_base_dir="./output/middle-result"):
    image_list = glob.glob(f"{image_dir}/*.png")
    # image_base_dir = os.path.dirname(image_dir).split("/")[-1]
    image_base_dir = os.path.basename(os.path.normpath(image_dir))
    # print(image_base_dir)

    output_dir = os.path.join(output_base_dir, image_base_dir)
    os.makedirs(output_dir, exist_ok=True)  

    for image_path in image_list:
        image = cv2.imread(image_path)
        masked_image, rows = hex_items_detect(image, hex_color="#ff0000")

        print(f"{image_path} - contour 개수: {len(rows)}")

        image_name = os.path.basename(image_path)
        save_path = os.path.join(output_dir, f"masked_{image_name}")
...
def hex_items_detect(image, hex_color="#ff0000", tolerance: int=80):
    """
    ...
    lower = np.array([max(0, b - tolerance) for b in bgr], dtype=np.uint8)
    upper = np.array([min(255, b + tolerance) for b in bgr], dtype=np.uint8)
    ...
    # else:
    #     valid_contours = []
    print(f"{image} - contour 개수: {len(valid_contours)}")
    ...
    rows = []
    for cnt in valid_contours:#valid_contours:
...
def compare_layouts(image1, image2, threshold=0.95):
    ...
    red_bgr = _hex_to_bgr("#ff0000")
    mask1 = np.all(img1 == red_bgr, axis=-1).astype(np.uint8) * 255
    mask2 = np.all(img2 == red_bgr, axis=-1).astype(np.uint8) * 255
...
def layout_classification(image_dir):
    ...
    for i, base_image in enumerate(image_list):
        ...
        for compare_image in image_list[i+1:]:
            ...
            result = compare_layouts(base_image, compare_image)
        ...
        # layout_name 디렉토리 생성
        layout_name = f"layout_{len(layout_groups)}"
        layout_dir = os.path.join(image_dir, layout_name)
        os.makedirs(layout_dir, exist_ok=True)

        for img_path in current_group:
            file_name = os.path.basename(img_path)
            save_path = os.path.join(layout_dir, f"{layout_name}_{file_name}")
            shutil.copy2(img_path, save_path)

        layout_groups[layout_name] = current_group
        print(f"{layout_name} 생성됨: {len(current_group)}개 이미지")
...
if __name__ == "__main__":
    ...
    # === 설정값 ===
    # RAW_IMAGE_DIR = "./output/visualization"
    # MASKED_IMAGE_DIR = "./output/xml-bbox"

    RAW_IMAGE_DIR = "./output/visualization"
    MASKED_IMAGE_DIR = "./output/middle-result"


    # split_by_ratio(RAW_IMAGE_DIR)

    # === 2. 비율별로 나뉜 디렉토리에서 마스킹 수행 ===
    layout_dirs = _layout_list(RAW_IMAGE_DIR)
    for layout_dir in layout_dirs:
        mask_images(layout_dir, output_base_dir=MASKED_IMAGE_DIR)

    # # === 3. 마스킹된 이미지 기준으로 레이아웃 분류 수행 ===
    # masked_layout_dirs = _layout_list(MASKED_IMAGE_DIR)  
    # for layout_dir in masked_layout_dirs:
    #     layout_classification(layout_dir)